...
`in_progress`. **U1-U9 completadas** (36 canarios, `node --test`) y **OpenBao integrado en el runtime** (el control plane guarda secretos en OpenBao, sin `NET_ADMIN`/`NET_RAW`; flujo E2E verificado). Falta: aplicar redaccion/RBAC/SSRF/auditoria en las superficies reales y el drill end-to-end del gate (canario no recuperable fuera del vault + restore utilizable).
